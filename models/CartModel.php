...
class CartModel extends BaseModel
{
    ...
    public function addOrUpdateItem(int $userId, int $productId, int $quantity = 1)
    {
        ...
        if ($ok) {
            $this->db->query("UPDATE {$this->cartTable} SET updated_at = NOW() WHERE id = $cartId");
        }
    ...
    public function updateItemQuantity(int $userId, int $productId, int $quantity)
    {
        ...
        $ok = $this->db->query($sql);
        if ($ok) $this->db->query("UPDATE {$this->cartTable} SET updated_at = NOW() WHERE id = $cartId");
        return (bool) $ok;
    }
    ...
    public function clearCartByUserId(int $userId)
    {
        ...
        if (!$cart) return false;
        $cartId = (int) $cart['id'];
        $ok = $this->db->query("DELETE FROM {$this->itemsTable} WHERE cart_id = $cartId");
        if ($ok) $this->db->query("UPDATE {$this->cartTable} SET updated_at = NOW() WHERE id = $cartId");
        return (bool) $ok;
    }
}
