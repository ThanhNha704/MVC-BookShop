        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10 items-start">
                ...
                <div class="md:col-span-1 flex justify-center">
                    <div class="w-full max-w-md bg-gray-50 rounded-lg overflow-hidden shadow-lg">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <?php if ($item['quantity'] <= 0): ?>
                                    <button type="button" disabled
                                        class="w-full bg-gray-400 text-white font-bold py-3 rounded-lg shadow cursor-not-allowed sm:col-span-2">
                                        <i class="fas fa-exclamation-triangle mr-2"></i> **HẾT HÀNG** - Vui lòng quay lại sau
                                    </button>
                                <?php else: ?>
                        ...
                        <div class="mt-6 border-b pb-6">
                            <?php if (isset($_SESSION['user_id'])): ?>
                                ...
                                <?php elseif ($canDelete): ?>
                                    <?php
                                    foreach ($data['reviews'] as $ur) {
                                        if ($ur['user_id'] == $_SESSION['user_id']) {
                                            $username = htmlspecialchars($ur['username'] ?? $ur['user_name']);
                                            $content = htmlspecialchars($ur['content'] ?? $ur['comment'] ?? '');
                                            $rating = isset($ur['rating']) ? (int) $ur['rating'] : 0;
                                            $status = $ur['status'] ?? 'Chờ duyệt';
                                        }
                                    }
                                    ?>
                                    <div class="mt-4"> Đánh giá của bạn: </div>
                                    <div class="border rounded-lg p-4 bg-gray-50">
                                        ...
                                        <div class="mt-2 text-xs text-gray-500"><?= htmlspecialchars($ur['created_at'] ?? '') ?>
                                            <form action="?controller=review&action=delete" method="POST" class="inline">
                                                <input type="hidden" name="review_id" value="<?= (int) $ur['id'] ?>">
                                                <button type="submit"
                                                    class="text-red-600 hover:underline text-xs ml-4">Xóa</button>
                                            </form>
                                        </div>
                                    </div>
                                <?php else: ?>
                        ...
                        <div class="mt-4 space-y-4">
                            <?php if (!empty($reviews)): ?>
                                <?php foreach ($reviews as $r):
                                    if (!isset($r['status']) || strtolower($r['status']) !== 'hiện')
                                        continue;
                                    ?>
                                    <?php
                                    $username = htmlspecialchars($r['username'] ?? $r['user_name']);
