<div class="flex flex-col md:flex-row justify-between items-center gap-4 mt-4">
    ...
    <div class="flex flex-wrap gap-2">
        <?php
        $categories = ['Tất cả', 'Văn học', 'Kỹ năng sống', 'Tâm lý', 'Kinh tế', 'Khoa học', 'Giải trí'];
        $selectedCategory = $_GET['category'] ?? 'Tất cả';
        foreach ($categories as $cat):
            $isActive = ($selectedCategory === $cat);
        ?>
            <a href="index.php?controller=product&action=index&category=<?= urlencode($cat) ?>"
               class="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 font-medium 
...
<div class="text-center mt-6 mb-4">
    <h2 class="text-3xl font-bold text-amber-600 uppercase tracking-wide">
        DANH SÁCH SẢN PHẨM
        <?php if ($selectedCategory && $selectedCategory !== 'Tất cả'): ?>
...
<div class="mx-auto product-list grid sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4 p-3 my-3">
    <?php if (!empty($noResults) && $noResults): ?>
    <div class="col-span-full text-center text-gray-500 text-lg font-medium p-6">
